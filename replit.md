...
- **Intelligent Auto-reply Engine**: A sophisticated `autoReply.ts` engine in the backend provides context-aware responses, detects user intent, recognizes product/game names, avoids repetition, and escalates to human agents when necessary, with consultation-type-adaptive greetings. For "unknown", "followup", "support_issue", "payment_question", "delivery_question", "trust_question", and generic "product_inquiry" intents, the engine delegates to an AI-powered module (`aiReply.ts`) that uses OpenAI gpt-4o-mini via direct API to generate contextual, business-aware responses in Chilean Spanish with gaming lingo. The AI has a comprehensive system prompt with deep knowledge of CJM Digitales: complete product catalog details, payment methods, purchase flow, activation guides (PS/Xbox), post-sale support (verification codes, activation issues, refunds), FAQ, and frustration/correction handling. Intent detection prioritizes post-purchase support and frustration patterns before product searches. AI can be toggled on/off globally via the admin panel Settings tab (stored in `app_settings` table, defaults to enabled).
