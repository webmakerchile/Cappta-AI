...
- **Intelligent Auto-reply Engine**: A sophisticated `autoReply.ts` engine in the backend provides context-aware responses, detects user intent, recognizes product/game names, avoids repetition, and escalates to human agents when necessary, with consultation-type-adaptive greetings. For "unknown" and "followup" intents, the engine delegates to an AI-powered module (`aiReply.ts`) that uses OpenAI gpt-4o-mini via Replit AI Integrations to generate contextual, business-aware responses in Chilean Spanish with gaming lingo. AI can be toggled on/off globally via the admin panel Settings tab (stored in `app_settings` table, defaults to enabled).
