...
*   **Decimal-aware money fields**: Payment-link amounts (`chat_payment_links.amount`) and agenda slot prices (`appointment_slots.price`) are stored as `double precision` (major units, not minor units / cents) so tenants on currencies with cents (USD, EUR, ARS, BRL, MXN, etc.) can charge values like 12.34. The `CurrencyInput` component, the `linkBodySchema` validator, and the Mercado Pago preference helper all derive the allowed decimal places from the tenant currency via `shared/currencies.ts` (`getCurrency().decimals`). Schema sync for fresh environments is handled by `npm run db:push` (drizzle-kit) — there is no SQL migration file for this column-type change.
*   **Multi-LLM por plan (GPT-only)**: Capa de abstracción en `server/llm/index.ts` con `chat()` y `chatStream()` que enruta a OpenAI (default `gpt-4o-mini`) y registra cada llamada en la tabla `llm_usage` (provider, model, kind, tokensIn/Out, costo en micros USD, latencia, status). Modelos permitidos por plan: free/solo/basic → solo `gpt-4o-mini`; scale/pro/enterprise → `gpt-4o-mini` + `gpt-4o`. Anthropic queda como stub (Claude Haiku/Sonnet) detrás de feature flag `LLM_ENABLE_ANTHROPIC=1` + `ANTHROPIC_API_KEY`. El tenant elige su modelo en Panel → Ajustes → Modelo de IA, persistido en `tenants.aiModel`. Endpoints: `GET /api/tenant-panel/llm/models`, `GET /api/tenant-panel/llm/usage?days=30`, `GET /api/admin/llm/usage` (admin). Callsites refactoreados: `server/aiReply.ts`, `server/knowledgeBase.ts`, `server/salesEngine/leadScorer.ts`. Endurecimiento de seguridad: `requireAuth` ahora rechaza explícitamente JWTs con `isTenant=true` para que tokens de tenants no puedan llegar a endpoints `/api/admin/*`.
*   **CurrencyInput browser tests**: Playwright suite at `tests/currency-input.spec.ts` drives a dedicated harness page (`/test/currency-input`, `client/src/pages/CurrencyInputHarness.tsx`) to lock in keyboard quirks across locales: `,`-vs-`.` decimal acceptance, ambiguous-paste heuristic (`1,234` vs `12,345`), and caret preservation when typing inside an already-formatted number. Run with `npx playwright test` (the dev server on port 5000 is auto-reused). The harness page is mounted in `client/src/App.tsx` next to other dev-only routes; it has no auth and no side effects.
